...
def read_video_frame_from_buffer(length):
    data=b""
    counter=0
    while counter<(length-length%10000):
        tmp=video_s.recv(10000)
        while len(tmp)<10000:
...
def receive_command():
    ...
    while True:
        if command_data_length>0:
            data=command_s.recv(command_data_length).decode()
            ...
            if data.startswith("VI:"):
                ...
                orig_width=int(data.split("VI:")[1].split(",")[0])
                orig_height=int(data.split("VI:")[1].split(",")[1])
                width=int(1920/5)
                height=int(1080/5)
                framerate=int(data.split("VI:")[1].split(",")[2])
                start_time=datetime.datetime.now()
                aspect_ratio=width/height
                display=pygame.display.set_mode((width,height),pygame.RESIZABLE | pygame.DOUBLEBUF)
...
def receive_video():
    ...
    while True:
        if video_data_length>0:
            ...
            frame_buffer.append([(s_width,s_height),read_video_frame_from_buffer(video_data_length)])
            video_data_length=0
        else:
